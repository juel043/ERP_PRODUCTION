...
if(isset($_POST['submit']))
  {
$purchasetitle=$_POST['purchasetitle'];
...
$sql="INSERT INTO tbl_purchase(PurchaseBy,PurchaseShop,PurchaseIteam,RequisitionTitle,PurchaseDetails,Price,Quantity,Mobile,T_Price) VALUES(:purchasetitle,:purchaseshop,:purchaseiteam,:requisitiontitle,:purchasedetails,:price,:quantity,:mobile,:t_price)";
...
$query->bindParam(':purchasedetails',$purchasedetails,PDO::PARAM_STR);
...
if($lastInsertId)
{
	$sql3="SELECT T_PriceRP FROM `tbl_requ_purch` WHERE id= $requisitiontitle";
    ...
    {				?>	
										<tr>
		                                 ...
		                                 <td><?php echo htmlentities($id1= $result->T_PriceRP);?></td>

										</tr> 
										<?php $cnt=$cnt+1;
										 echo '<script>' .
          'console.log("'.$id1 .' ")</script>'; 
      }}

      $t_due = $id1-$t_price; 
         echo '<script>' .
          'console.log("'.$t_due .' ")</script>';

	  $sql2= "UPDATE tbl_requ_purch SET pur_t_price=:t_price,t_due=:t_due WHERE id=:requisitiontitle";
    $query = $dbh->prepare($sql2);
    $query->bindParam(':requisitiontitle',$requisitiontitle,PDO::PARAM_STR);
    $query->bindParam(':t_price',$t_price,PDO::PARAM_STR);
    $query->bindParam(':t_due',$t_due,PDO::PARAM_STR);
    $query->execute();
    $lastInsertId = $dbh->lastInsertId();


 
    $msg="Purchase Upadtae successfully";
...
<div class="col-sm-4">
<input type="text" name="purchasetitle" class="form-control" required>
</div>
...
<div class="col-sm-4">
<input type="text" name="purchaseshop" class="form-control" required>
</div>
...
{
?>
<option value="<?php echo htmlentities($result->id);?>"><?php echo htmlentities($result->RequisitionTitle);?></option>
<?php }} ?>
...
<div class="col-sm-10">
<textarea class="form-control" name="purchasedetails" rows="3" required></textarea>
</div>
</div>
